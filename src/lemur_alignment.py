...
def stack_corrected_slice(folder : str, *, scale_ratio:int = 16, brain_info: dict = None):
    slice_folder = os.path.join(folder, 'background_corrected')
    (_, _, filenames) = next(os.walk(os.path.join(folder)))

    prefix = re.split('^(.*)_([0-9]{2})_scene([0-9])_background_corrected.nim$', filenames[0])[1]
    slice_list = [int(re.split('^(.*)_([0-9]+).czi$', fn)[2]) for fn in filenames]
    slice_list = np.sort(slice_list)

    if brain_info is None:
        info_tuple = list(zip(info['filename'], info['scene']))
        filenames = [os.path.join(slice_folder, f'{os.path.basename(fn)[:-4]}_scene{scene}_background_corrected.nim')
                     for (fn, scene) in info_tuple]
...
def export_matching_reference(reflabel_name: str, movlabel_name: str, result_filename: str, result_folder: str,
    ...
    # Group matching
    group_matched_ref = np.zeros((ants_matched_ref.shape[0],ants_matched_ref.shape[1], len(region_group)), dtype='uint8')    
    slice_idx = 0
...
def apply_rigid_tranform_image(result_filename: str, tform_folder: str, reflabel_name: str, movlabel_name: str, czi_folder : str, lemur_prefix : str, *, group_split: list = None, group_flip: list = None, group_error: list = None, czi_filename : str = None, run_shading_correction: bool = False, brain_info : dict = None): 
...
if __name__ == "__main__":
    ...
    for idx in [6]:
        ...
        apply_rigid_tranform_image(result_filename, tform_folder, reflabel_name, movlabel_name, czi_folder, lemur_prefix, group_split=group_split, run_shading_correction = False, group_flip = group_flip, group_error = group_error, brain_info = brain_info)

...
def get_slice_centroid(ra_dict: dict, slice_idx: int, x_axis: int):
                ...
                if need_split:
                    if midline_filename is not None:
                        ...
                        if midline_x is not None:
                            ...
                            if x_start + compact_mask.shape[1] / 2 > get_slice_centroid(ra_dict, img_slice) * scale_down:  # right side
                                annotation_mask[img_slice][mask] = mapped_region_id + 10
                            else:
                                annotation_mask[img_slice][mask] = mapped_region_id
                else:
...
def export_grouped_label_img_other(img_filename: str, ra_filename: str,
                             region_group: list, group_split: list,
                             result_filename: str, *,  downsample_ratio: int = 16):
    read_ratio = downsample_ratio
    scale_down = 1.0 / read_ratio  # otherwise the mask will be too big
    ...
    for region_id, slice_rois in region_to_masks.items():
        if region_id not in region_id_set:
            continue
        for img_slice, maskps in slice_rois.items():
            logger.info(f'running {img_slice}')
            mapped_region_id = 0
            for group_idx, group in enumerate(region_group[img_slice], start=1):
                if region_id in group:
                    mapped_region_id = group_idx
            ...
            for compact_mask, x_start, y_start, spline in maskps:
                ...
                if need_split:
                    if img_rotate:
                        # if x_start + compact_mask.shape[1] / 2 > get_slice_centroid(ra_dict, img_slice, 1):  # right side
                        if np.mean(spline[0]['Points'][:,1]) > get_slice_centroid(ra_dict, img_slice, 1):  # right side
                            annotation_mask[img_slice][mask] = mapped_region_id + 10
                        else:
                            annotation_mask[img_slice][mask] = mapped_region_id
                    else:
                        if np.mean(spline[0]['Points'][:,0]) > get_slice_centroid(ra_dict, img_slice, 0):  # right side
                            annotation_mask[img_slice][mask] = mapped_region_id + 10
                        else:
                            annotation_mask[img_slice][mask] = mapped_region_id
                else:
    ...
    if img_rotate:
        img_util.write_img(result_filename, np.moveaxis(annotation_mask, -1, -2))
    else:   
        img_util.write_img(result_filename, annotation_mask)

def main():
    # list (slice) of list (groups) of list of number (regions in group)
    region_group_name = '/Users/hyungju/Desktop/hyungju/Result/lemur-blockface/mesh-align/all-dataset/Hotsauce_PV_TH_NeuN/region_group.txt'
    with open(region_group_name) as json_file:
        region_group = json.load(json_file)

    # list (slice) of list of number (groups to be split)
    group_split_name = '/Users/hyungju/Desktop/hyungju/Result/lemur-blockface/mesh-align/all-dataset/Hotsauce_PV_TH_NeuN/group_split.txt'
    ...
    ra_filename = os.path.join(folder, 'interns_edited_results', 'edited_merge_20201001_2.reganno')
    result_filename = os.path.join(folder, 'interns_edited_results', 'edited_merge_20201001_2_merged_label.nim')
    midline_filename = os.path.join(folder, 'interns_edited_results', 'sh_cut_in_half.reganno')
    # export_grouped_label_img(img_filename, ra_filename, region_group, group_split,result_filename, midline_filename=midline_filename)

    ra_filename = os.path.join(folder, 'fit', 'fit4', 'edited_merge_20201001_2_merge_align_fixed.reganno')
    result_filename = os.path.join(folder, 'fit', 'fit4', 'edited_merge_20201001_2_merge_align_fixed_merged_label.nim')
    # export_grouped_label_img(img_filename, ra_filename, region_group, group_split,result_filename, midline_x=28080 / 2.0)
      
    folder = os.path.join(lemur_folder, 'Hotsauce_334A', '181016_Lemur-Hotsauce_PV_TH_NeuN')
    ra_filename = os.path.join(folder, 'Lemur-H_PV_TH_NeuN_combined.reganno')
    result_filename = '/Users/hyungju/Desktop/hyungju/Result/lemur-blockface/mesh-align/all-dataset/Hotsauce_PV_TH_NeuN/01_grouped_bigregion_label.nim'
    img_filename = '/Users/hyungju/Desktop/hyungju/Result/lemur-blockface/mesh-align/all-dataset/Hotsauce_PV_TH_NeuN/00_stacked_label.nim'
    export_grouped_label_img_other(img_filename, ra_filename, region_group, group_split, result_filename)
